...
const NetworkType = () => {
	...
	const handleAddNetworkType = async (val: Partial<NetworkInformation>) => {
		const { createNetworkType } = await _fetch({ createNetworkType: val })

		console.log(createNetworkType)

		if (createNetworkType) {
			const { success, data, errmsg } = createNetworkType
